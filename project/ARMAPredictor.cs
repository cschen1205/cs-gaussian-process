...
namespace SimuKit.ML.TimeSeries
{
    ...
    public class ARMAPredictor : Predictor
    {
        private int mAROrder = 1; //auto regression order
        private int mMAOrder = 1; //moving average order
        ...
        {
            return string.Format("ARMA: MA_Order={0}, AR_Order={1}, OptIter = {2}, LDSIter = {3}", mMAOrder, mAROrder, OptIters, LDSIters);
        }

        public override Predictor Clone()
        {
            ARMAPredictor p = new ARMAPredictor();
            p.MAOrder = mMAOrder;
            p.AROrder = mAROrder;
            p.LDSIters = LDSIters;
            p.OptIters = OptIters;
